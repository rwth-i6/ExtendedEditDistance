...
def EED(hyp, ref):
    _EED = ctypes.CDLL('/mnt/d/Work/EED-Git/eed/libEED.so')
    _EED.wrapper.restype = ctypes.c_float
    hyp.insert(0, " ")
    hyp.append(" ")
    ref.insert(0, " ")
    ref.append(" ")
    hyp_c = (ctypes.c_int * len(hyp))()
    hyp_c[:] = [bytes_to_int(x.encode('utf-8')) for x in hyp]
    ref_c = (ctypes.c_int * len(ref))()
    ...
    result = _EED.wrapper(hyp_c, ref_c, len(hyp_c), len(ref_c), ctypes.c_float(alpha), ctypes.c_float(deletion), ctypes.c_float(insertion), ctypes.c_float(rho))
...
def EED_Python(hyp, ref):
    ...
    return (row[-1]+ coverage)/(float(len(ref)) + coverage)
...
def main():
    ...
    import codecs
    args = parse_args()
    hlines = [util.preprocess(x) for x in codecs.open(args.hypothesis, 'r', 'utf-8').readlines()]
    rlines = [util.preprocess(x) for x in codecs.open(args.reference, 'r', 'utf-8').readlines()]
